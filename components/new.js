...
export function New({user}) {
    ...
    async function handleSubmit(user) {
        ...
        if (now.month < date.start.month && now.year <= date.start.year) {
            status = 'Upcoming'
        }
        if (now.month > date.end.month && now.year >= date.end.year) {
            status = 'Unfinished'
        }
        if (now.month === date.end.month && now.year === date.end.year) {
            if (now.day > date.end.day) {
                status = 'Unfinished'
            }
        }
        if (now.month === date.start.month && now.year === date.start.year) {
            if (now.day < date.start.day) {
                status = 'Upcoming'
            }
        } 
